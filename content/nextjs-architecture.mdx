...
- **Layer 3 — Authorization.** Authorization itself decomposes into **three permission planes** that must be checked together. Treating them as one boolean is how leaks happen.

#### The three permission planes

| Plane | Question it answers | Where it lives | Resolves from |
| --- | --- | --- | --- |
| **1. Operation (RBAC)** | "Can this user perform this action at all?" | User's role/policy | `user.permissions: Set<string>` of `resource:operation` strings, with wildcard support (`agent:*`, `*:*`) |
| **2. Resource** | "Can this user touch *this specific* object?" | The object itself | Ownership, visibility (`private` / `org` / `public`), tenant/org isolation |
| **3. Project / Workspace** | "Within this project, what can this user do?" | Project membership | Per-project role merged with user-level perms via **union strategy** |

A request must clear **all three planes** to proceed. The planes are independent — RBAC alone doesn't grant access to someone else's private resource; project membership alone doesn't grant operations the user's role forbids.
...
export type UserContext = {
  ...
  externalId: string
  orgId: string
  perms: Set<Action>                 // plane 1 — RBAC
  projects: Map<string, ProjectMembership> // plane 3 — per-project memberships
}
```
...
// Plane 1 — operation (RBAC) with wildcard support
...
// Plane 2 — resource (ownership / visibility / org isolation)
...
// Plane 3 — project membership (UNION with user-level perms)
...
// Composed gate — all three planes evaluated together
export function can(ctx: UserContext, action: Action, res?: Resource): boolean {
  ...
  return hasOperation(ctx.perms, action) || projectGrants(ctx, res, action) // plane 1 ∪ plane 3
}
...
```ts
// service-layer usage — the three planes are invisible to business code
...
**Why the union strategy on plane 3.** A user who is normally a `viewer` in the org may be an `editor` in a specific project — their effective perms inside that project are the **union** of org-level and project-level perms. Intersection (the conservative choice) breaks legitimate collaboration patterns; union with explicit deny rules is the practical default.
...
- **Plane 2 (resource)** must be checked **after** loading the entity — it requires the entity's `ownerId` / `visibility` / `orgId`. Always in the service layer.
- **Plane 3 (project)** is checked alongside plane 1 inside the service layer for project-scoped resources.

**Pitfalls to avoid:**

- **Don't check plane 1 alone** when an entity is involved — always pair with plane 2.
- **Don't return 404 for "exists but forbidden"** unless leaking existence is itself a problem (it often is — prefer 404 for private resources, 403 for org-visible resources).
- **Wildcard discipline.** `*:*` should only ever belong to system/admin tokens, never to regular users. Audit who holds it.
- **Dual-ID pitfall.** If your system has both an internal user id and an external id from an upstream IDP, name them explicitly (`id` vs `externalId`) and document which one each table foreign-keys to. Project membership and resource ownership often disagree on which id they store — one of the most common silent multi-tenant bugs.

### 10. Cross-Cutting Concerns via AOP

**Principle.** Tracing, metrics, usage, rate limiting, logging, and health monitoring are **cross-cutting** — they apply to every request and must not pollute business code. Compose them transparently via HOFs / decorators / middleware, and treat **what you can see** as a first-class architectural concern, not a post-hoc add-on.
...
HOF wrappers stack like middleware around routes and server actions. The order matters — outer wrappers see the request first and the response last:
...
export const POST = composeAPIRoute([
  ...
  mapErrors,                                                // converts BizError → HTTP (see §8)
  authUser,                                                 // attaches UserContext (see §9)
  withUsage('elaborator.create'),                           // emits a usage measure on success
  async (req, ctx) => { /* business */ }
])
```

#### Logging — prefixed, structured, PII-safe
...
2. **Prefixed by scope.** Every logger carries a `[Scope]` prefix derived from the module — instantly readable in tail/grep, and machine-filterable in log search.
...
// PII helper — mirrors the Python `%p` placeholder convention.
...
// → [AuthMiddleware] Token validated successfully for user: a3f9c1b2  {"scope":"AuthMiddleware","requestId":"..."}
```
...
- Always include `requestId` and `tenantId` in log meta — propagated via `AsyncLocalStorage` (see §12).
...
- An `eslint-plugin` rule (or PR review checklist) flagging `console.log` in `app/` and `services/` — the prefixed logger is the only allowed surface.

#### Tracing — distributed spans around business operations

Initialize OpenTelemetry **once** in `instrumentation.ts` (see §3). Span names are **business-meaningful**, not URL paths — `agent.publish`, `elaborator.search`, `llm.completion`, not `POST /api/v1/agents/:id/publish`.
...
// Usage — wrap a service method with a span and a small set of attributes
...
- **Span names** form a stable taxonomy — treat them like an enum (typed `SpanName` union).
- **Attributes** are low-cardinality (`tenant.id` ok; raw email NOT ok — also a PII issue).
- Wrap **service-layer methods**, not utility functions — spans should mean something to a person reading a trace.
- Propagate `traceparent` headers through outbound `fetch` automatically via the OTel HTTP instrumentation.

#### Metrics — typed enum, never free-form strings
...
export function record(name: MetricName, value: number, tags?: Record<string, string>) {
  // Datadog / Prometheus / OTel meter — pick one, hide it here
...
**Tag discipline.** Tags are low-cardinality (`tenant_tier`, `route`, `model`) — never high-cardinality values like raw user ids or full URLs. Each unique tag combination becomes a billed time series.
...
Wire it in `instrumentation.ts` (§3). Alert when p99 exceeds your SLO budget — it's the earliest, cheapest signal that something is wrong on the server.

Other health signals worth wiring at the same time:

- **Active handles / requests** count (`process._getActiveHandles().length`) — leaks show as monotonic growth.
...
Domain-meaningful counters that bill, throttle, or analytics-track an action — emit them as their own primitive, not buried in logs:
...
> You can install [next.js BP skill](https://skills.sh/vercel-labs/next-skills/next-best-practices) from Vercel Official Skills Hub to instruct AI for BP. 🗞️
...
9. **Three-layer auth + three-plane permissions**: `middleware.ts` (authn) → `getUserContext()` (identity) → `can()` resolver evaluating **operation (RBAC) ∧ resource (ownership/visibility/org) ∧ project (membership union)** (§9).
10. **Observable by construction**: HOF-composed `withSpan` / `withUsage` / `withRateLimit`, `getPrefixedLogger(scope)` with PII redaction + `safe.*` helpers, typed `Metric` enum, and event-loop lag (`monitorEventLoopDelay`) as a first-class health signal (§10).
