---
title: '🪄 Arno Agentic Coding BP for Developers 2026'
publishedAt: '2026-02-26'
summary: 'A comprehensive guide to Agentic Coding best practices for full-stack developers, covering prompt engineering, tool-specific context, human oversight, proactive quality management, and continuous learning.'
...
- **Prompts -> Context engineering** its all about prompt properly and manage context in a good way. Make full use of `AGENTS.md`, `SKILLS.md` (skills for knowledge and SOP / workflow definition), `TOOLs / MCPs / Hooks` for enhancements the capabilities of AI.
...
- **Iterate fast, fail fast** find the way to success fast, it's all about **Speed**. Do VibeCoding / Agentic coding often with best AI Models and tools and try **Automate everything you can find with AI**.
...
* init `CLAUDE.md` or `AGENTS.md` file to setup your agentic system prompt and context, you can use `/init` command to do this.
* use `/plugins`, `/mcp` and `/skills` to add your favorite plugins, and your own skills and rules.
* use `/model` to switch models for proper tasks.

Here are the [Claude Code BP](/knowledge/ai/claude-code-bp) before you start.
...
> for Cursor, `CLAUDE.md` is compatible, just use it as your global rules and instructions.
...
you can see the [Spec-Kit + Spec-Driven Design (SDD) — Simple BP Guide](/posts/spec-driven-dev) for more details.
...
* xcode + vscode + agentic tools together, vibe-coding / agentic-coding in vscode and debug in xcode
...
'''
I want to find the BP for Vibe / Agentic Coding in Cursor or Github Copilot, what is the best practice for full-stack developer?
...
- don't hesitate to use more `@` symbol to add accurate context for your ai prompts
...
- agentic-coding sometimes go astray, let's fallback to original coding experience with AI guide in more detailed operations way.
...
* (26-03-15) better inspired by Claude Code BP and Cursor BP, and add more content and examples.
